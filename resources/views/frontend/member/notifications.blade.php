                ...
                <div class="btn-group mb-2">
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <a href="{{ route('notification.mark_all_as_read') }}"
                                class="dropdown-item">{{ translate('Mark all as read') }}</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                @if (!$notifications->isEmpty())
                    @foreach ($notifications as $notification)
                        ...
                        @if ($notify_data->type == 'express_interest')
                            @php
                                $interest_data = App\Models\ExpressInterest::where(
                                    'id',
                                    $notify_data->info_id,
                                )->first();
                            ...
                            <li
                                ...
                                <a href="{{ route('notification_view', $notification->id) }}" class="media text-inherit">
                                    ...
                                    <span class="avatar avatar-sm mr-3">
                                        <img
                                            @if ($profile_picture_show || $user->user_type == 'admin') src="{{ uploaded_asset($user->photo) }}"
                                            @else
