<div class="chat-box-wrap h-100">
    <div
        ...
        <div class="d-flex align-items-center">
            ...
            <button class="btn btn-icon btn-circle btn-soft-primary chat-info" data-toggle="class-toggle"
                data-target=".chat-info-wrap"><i class="fa-solid fa-info-circle"></i></button>
        </div>
    </div>
    <div class="chat-list-wrap c-scrollbar-light scroll-to-btm" id="parentDiv">
        @if (count($chats) > 0)
            <div class="chat-coversation-load text-center">
                <button class="btn btn-link load-more-btn" data-first="{{ $chats->last()->id }}"
                    type="button">{{ translate('Load More') }}</button>
            </div>
        @endif
    ...
    <div class="chat-info-wrap">
        <div class="overlay dark c-pointer" data-toggle="class-toggle" data-target=".chat-info-wrap"
            data-same=".chat-info"></div>
        <div class="chat-info c-scrollbar-light p-4 z-1">
            <div class="px-4 text-center mb-3">
                ...
                <h4 class="h5 mb-2 fw-600">
                    {{ $chat_thread->$user_to_show->first_name . ' ' . $chat_thread->$user_to_show->last_name }}</h4>
            </div>
            <div class="text-center">
                <h6 class="fs-13">{{ translate('Age') }}:
                    {{ \Carbon\Carbon::parse($chat_thread->$user_to_show->member->birthday)->age }}</h6>
                <h6 class="fs-13">
                    {{ translate('Height') }} :
                ...
                <div class="text-center mb-3 px-3 mt-3">
                    <a @if (get_setting('full_profile_show_according_to_membership') == 1 && Auth::user()->membership == 1) href="javascript:void(0);" onclick="package_update_alert()"
                            @else
                                href="{{ route('member_profile', $chat_thread->$user_to_show->id) }}" @endif
