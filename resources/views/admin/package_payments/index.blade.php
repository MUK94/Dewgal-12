                    ...
                    <table class="table aiz-table mb-0">
                        ...
                        <tbody>
                            @foreach ($package_payments as $key => $package_payment)
                                <tr>
                                    <td>{{ $key + 1 + ($package_payments->currentPage() - 1) * $package_payments->perPage() }}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @if ($package_payment->payment_status == 'Paid')
                                            <span
                                                class="badge badge-inline badge-success text-center">{{ translate('Paid') }}</span>
                                        @else
                                            <span
                                                class="badge badge-inline badge-danger text-center">{{ translate('Unpaid') }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-right">
                                        @if ($package_payment->payment_method == 'manual_payment' && auth()->user()->can('manage_package_manual_payemnts'))
                                            <a href="javascript:void(0);"
                                                onclick="package_payment_details('{{ route('package-payments.show', $package_payment->id) }}')"
                                        ...
                                        @can('view_package_payment_invoice')
                                            <a href="{{ route('package_payment.invoice_admin', $package_payment->id) }}"
                                                target="_blank" class="btn btn-soft-info btn-icon btn-circle btn-sm"
