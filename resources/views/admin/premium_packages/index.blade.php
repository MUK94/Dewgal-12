                        ...
                        <tbody>
                            @foreach ($packages as $key => $package)
                                <tr>
                                    ...
                                    <td>
                                        <img class="img-md" src="{{ uploaded_asset($package->image) }}" height="45px"
                                            alt="{{ translate('photo') }}">
                                    </td>
                                    <td>{{ $package->name }}</td>
                                    <td>{{ $package->price }}</td>
                                    <td>
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input type="checkbox" id="status.{{ $key }}"
                                                onchange="update_package_activation_status(this)"
                                                value="{{ $package->id }}"
                                                @if ($package->active == 1) checked @endif data-switch="success"/
                                    ...
                                    <td class="text-right">
                                        @can('edit_package')
                                            <a class="btn btn-soft-info btn-icon btn-circle btn-sm"
                                                href="{{ route('packages.edit', encrypt($package->id)) }}"
                                        ...
                                        @if ($package->id != 1 && auth()->user()->can('delete_package'))
                                            <a href="javascript:void(0);"
                                                ...
                                                data-href="{{ route('packages.destroy', $package->id) }}"
