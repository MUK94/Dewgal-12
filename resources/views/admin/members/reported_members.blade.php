                            ...
                            <td class="text-right">
                                @can('block_member')
                                    @if ($report->user->blocked == 0)
                                        <a href="javascript:void(0);" onclick="block_member({{ $report->user_id }})"
                                ...
                                @can('delete_profile_report')
                                    <a href="javascript:void(0);"
                                        data-href="{{ route('report_destrot.destroy', $report->id) }}"
                ...
                <form class="form-horizontal member-block" action="{{ route('members.block') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <textarea type="text" name="blocking_reason" class="form-control" placeholder="{{ translate('Blocking Reason') }}"
                                    required></textarea>
                            </div>
                        </div>
                    </div>
