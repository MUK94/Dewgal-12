                    ...
                    <table class="table aiz-table mb-0">
                        ...
                        <tbody>
                            @foreach ($deleted_members as $key => $deleted_member)
                                <tr>
                                    <td>{{ $key + 1 + ($deleted_members->currentPage() - 1) * $deleted_members->perPage() }}
                                    </td>
                                    <td>
                                        {{-- @if (uploaded_asset($deleted_member->photo) != null)
                                            <img class="img-md" src="{{ uploaded_asset($deleted_member->photo) }}"
                                    ...
                                    <td>
                                        @if ($deleted_member->reported_users->count() > 0)
                                            <a href="{{ route('reported_members', $deleted_member->id) }}"
                                                class="badge badge-inline badge-danger"
                                                title="{{ translate('View Reports') }}">{{ $deleted_member->reported_users->count() }}</a>
                                        @else
                                            0
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-right">
                                        @can('restore_member')
                                            <a class="btn btn-soft-success btn-icon btn-circle btn-sm"
                                                href="{{ route('restore_deleted_member', $deleted_member->id) }}"
                                                title="{{ translate('Restore') }}">
                                                <i class="fa-solid fa-check-circle"></i>
                                            </a>
                                        @endcan
                                        <a href="javascript:void(0);"
                                            data-href="{{ route('members.permanently_delete', $deleted_member->id) }}"
