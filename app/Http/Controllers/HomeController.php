    ...
    public function email_change_callback(Request $request)
    {
        if ($request->has('new_email_verificiation_code') && $request->has('email')) {
            ...
            if ($user != null) {
                ...
                return redirect()->route('dashboard')->with('success', translate('Email Changed successfully'));
            }
        }

        return redirect()->route('dashboard')->with('error', translate('Email was not verified. Please resend your mail!'));
    }
    ...
    public function reset_password_with_code(Request $request)
    {
        if (($user = User::where('email', $request->email)->where('verification_code', $request->code)->first()) != null) {
            if ($request->password == $request->password_confirmation) {
                ...
                return back()->with('warning', "Password and confirm password didn't match");
            }
        } else {
