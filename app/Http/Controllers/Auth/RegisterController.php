    ...
    public function register(Request $request)
    {
        if (filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
            if (User::where('email', $request->email)->first() != null) {
                ...
                session()->flash('message', translate('Email or Phone already exists.'));
                return back();
            }
        } elseif (User::where('phone', '+' . $request->country_code . $request->phone)->first() != null) {
            // flash(translate('Phone already exists.'));
            session()->flash('message', translate('Phone already exists.'));
            return back();
        }
        ...
        if (get_setting('email_verification') == 0) {
            if ($user->email != null || $user->phone != null) {
                ...
                session()->flash('success', translate('Registration successfull.'));
            }
        } else if ($user->email != null) {
            event(new Registered($user));
            // flash(translate('Registration successfull. Please verify your email.'))->success();
            session()->flash('success', translate('Registration successfull. Please verify your email.'));
        } else {
            // flash(translate('Registration successfull. Please verify your phone number.'))->success();
            session()->flash('success', translate('Registration successfull. Please verify your phone number.'));
        }
